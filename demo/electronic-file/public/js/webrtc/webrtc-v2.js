...
const PHONE = window.PHONE = config => {
    ...
    const mediaconf     = config.media || { audio : true,
      video : { mandatory: { maxWidth: 640, maxHeight: 480,maxFrameRate:60}}
      // video : {
      //     mandatory: {
      //       width: { max: 640 },
      //       height: { max: 480 }
      ...
      //     ]
      //   }
    };//{ mandatory: { maxWidth: 1280, maxHeight: 720,minWidth: 640, minHeight: 480 }}
    const conversations = {};
    ...
    {
        "url":"stun:183.250.15.3:3478","credential":"2016tianmeng","username":"admin"
    }
    ,{
        "url":"turn:183.250.15.3:3478","credential":"2016tianmeng","username":"admin"
    }
    ,{
        "url":"stun:120.76.22.45:3478","credential":"2016tianmeng","username":"admin"
    }
    ,{
        "url":"turn:120.76.22.45:3478","credential":"2016tianmeng","username":"admin"
    }
    ] };
    ...
    function receive(message) {
        ...
        //console.log(message.packet.extra);// != "" && message.packet.extra !=undefined
        if(message.packet.extra){
            extracb(message.packet.extra);
        }
        // Get Call Reference
        let talk = get_conversation(message.number);
        console.log("debug-task=",talk);
        // Ignore if Closed
        if (talk.closed) return;
